...
<div class="loading" v-if="isLoading">
  <img id="beer" src="C:\Users\Student\workspace\brewery-finder-capstone\capstone\vue\public\giphy.gif"/> 
  </div>
...
<label for="beerType">Type of Beer</label>
    <input type="text" id="beerType" name="type" placeholder="Type of the beer">

    <label for="beerDesc">Description</label>
    <input type="text" id="beerDesc" name="description" placeholder="Description..">

<label for="beerABV">ABV</label>
    <input type="text" id="beerABV" name="ABV" placeholder="ABV %">
...
<label for="Type">Brewery ID</label>
    <input type="text" id="info" name="ABV" placeholder="Brewery ID">
...
<div class="show-form-button">
    ...
    v-if="showForm === false && this.$store.state.user.id === this.brewery.Id">
    Show Form </button>
...
export default {
  ...
  methods: {
    addNewBeer(){
      applicationService.addNewBeer(this.newBeer).then(response => {
    ...
    updateBeer(beer){
applicationService.updateBeer(beer).then(response => {
  if(response.status === 201){
